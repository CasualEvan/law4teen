...
function HeadlinesTab() {
  ...
  const handleSave = async () => {
    ...
    await supabase.from("headlines").insert(headlines.map((h, i) => ({
      title: h.title, summary: h.summary, category: h.category, time: h.time, impact: h.impact, position: i,
    })));
    setSaving(false); setSaved(true);
    setTimeout(() => setSaved(false), 2500);
    fetchHeadlines();
  ...
  const handleReset = async () => {
    if (!window.confirm("Reset headlines to defaults?")) return;
    setSaving(true);
    await supabase.from("headlines").delete().neq("id", 0);
    await supabase.from("headlines").insert(defaultHeadlines);
    setSaving(false); fetchHeadlines();
  };

  const handleChange = (i, field, value) => setHeadlines(prev => prev.map((h, idx) => idx === i ? { ...h, [field]: value } : h));
  const handleAdd = () => { setHeadlines(prev => [...prev, { title: "", summary: "", category: "", time: "", impact: "Medium", position: prev.length }]); setExpanded(headlines.length); };
  const handleDelete = (i) => { setHeadlines(prev => prev.filter((_, idx) => idx !== i)); setExpanded(null); };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <p className="font-play text-sm text-muted-foreground">{headlines.length} curated headline{headlines.length !== 1 ? "s" : ""} — shown above live court opinions</p>
        <div className="flex items-center gap-2">
          <button onClick={handleReset} disabled={saving} className="font-play text-sm text-muted-foreground hover:text-red-500 transition-colors px-3 py-1.5 rounded-lg border border-border hover:border-red-200 disabled:opacity-50">Reset</button>
          <button onClick={handleSave} disabled={saving} className={`flex items-center gap-2 font-play font-bold text-sm px-4 py-2 rounded-lg transition-all disabled:opacity-50 ${saved ? "bg-green-500 text-white" : "bg-navy-dark text-gold hover:bg-navy-light"}`}>
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : saved ? <CheckCircle className="w-4 h-4" /> : <Save className="w-4 h-4" />}
            {saving ? "Saving..." : saved ? "Saved!" : "Save Changes"}
          </button>
          ...
          <div key={i} className="bg-white border border-border rounded-xl overflow-hidden">
            <div onClick={() => setExpanded(expanded === i ? null : i)} className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-muted/20 transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="font-display font-bold text-navy-dark text-sm truncate">{h.title || <span className="text-muted-foreground italic font-play font-normal">Untitled</span>}</span>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0 ml-3">
                <span className={`text-xs font-play px-2 py-0.5 rounded border font-medium ${h.impact === "High" ? "bg-red-100 text-red-700 border-red-200" : h.impact === "Low" ? "bg-green-100 text-green-700 border-green-200" : "bg-gold/10 text-gold-dark border-gold/20"}`}>{h.impact}</span>
            ...
            {expanded === i && (
              <div className="border-t border-border px-5 py-5 space-y-4">
                <div><label className="font-play text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Title</label>
                  <input type="text" value={h.title} onChange={e => handleChange(i, "title", e.target.value)} className="w-full border border-border rounded-lg px-3 py-2 font-play text-sm focus:outline-none focus:ring-2 focus:ring-navy-dark" /></div>
                <div><label className="font-play text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Summary</label>
                  <textarea value={h.summary} onChange={e => handleChange(i, "summary", e.target.value)} rows={3} className="w-full border border-border rounded-lg px-3 py-2 font-play text-sm focus:outline-none focus:ring-2 focus:ring-navy-dark resize-none" /></div>
                <div className="grid grid-cols-3 gap-3">
                  <div><label className="font-play text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Category</label>
                    <input type="text" value={h.category} onChange={e => handleChange(i, "category", e.target.value)} className="w-full border border-border rounded-lg px-3 py-2 font-play text-sm focus:outline-none focus:ring-2 focus:ring-navy-dark" /></div>
                  <div><label className="font-play text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Time</label>
                    <input type="text" value={h.time} onChange={e => handleChange(i, "time", e.target.value)} className="w-full border border-border rounded-lg px-3 py-2 font-play text-sm focus:outline-none focus:ring-2 focus:ring-navy-dark" placeholder="e.g. 2 hours ago" /></div>
                  <div><label className="font-play text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">Impact</label>
                ...
                <div className="flex justify-end"><button onClick={() => handleDelete(i)} className="flex items-center gap-1.5 text-red-500 hover:text-red-700 font-play text-sm transition-colors"><Trash2 className="w-4 h-4" /> Delete</button></div>
              </div>
            )}
          </div>
        ))}
